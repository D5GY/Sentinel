...
export default class KickCommand extends Command {
	...
	async run(message: Message, args: CommandArguments, send: SendFunction) {
		...
		if (!member || member.id === message.author.id) throw noMemberError;
		if (!Util.isManageableBy(member, message.member!)) throw new CommandError('NOT_MANAGEABLE', 'kick');
		if (!member.kickable) throw new CommandError('NOT_MANAGEABLE', 'kick', { byBot: true });
